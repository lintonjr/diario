    ...
    <table id="tabela-item-lista" class="table table-fixed table-striped table-hover table-middle mb-0">
      <thead>
        <tr>
          ...
          <th><a href="#" class="order" order="down" sort="ASC">Download</a></th>
          <th class="tools"></th>
        </tr>
      </thead>
      <tbody class="scrollbar-inner">
        @forelse($itens as $i)
          <tr>
            <td>{{\Helper::formatDate($i->dailies->date_released)}}</td>
            <td>{{ $i->title }}</td>
            <td>{{ $i->dailies->id }}</td>
            <td><a href="{{ url("$i->file") }}" target="_blank" class="bqX"> <i class="mdi mdi-download"></i> </a></td>
            <td class="tools">
              <ul class="bqY">
